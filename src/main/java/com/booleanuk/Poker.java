...
public class Poker {
    ...
    public boolean isThreeOfAKind(String[] hand){
        int i = 0;
        Arrays.sort(hand);
        if (hand.length >= 3){
            return (hand[i].equals(hand[i + 1]) && hand[i].equals(hand[i + 2]));
        }
        return false;
    }
    ...
    public String[] winningThreeCardHand(String[][] hands) {
        ...
        for (String[] hand : hands) {
            if (isThreeOfAKind(hand)) {
                currentHand = hand;
                winningHand = winningThreeCardHandHelper(winningHand, currentHand);
            }else if(isPair(hand)){
                currentHand = hand;
                winningHand = winningPair(winningHand, currentHand);
            }
        }
        if (isThreeOfAKind(winningHand)){
            return winningHand;
        }
        else if (isPair(winningHand)){
            return winningHand;
        }
        return new String[]{};
    }
    ...
    public String[] winningThreeCardHandHelper(String[] firstHand, String[] secondHand) {
        // Implement the winningPair logic here and return the array containing the winning pair to make the tests pass.
        // You can replace the following return value with something appropriate
        int totalvalue1 = 0;
        int totalvalue2 = 0;
        ...
        if (isThreeOfAKind(firstHand) && isThreeOfAKind(secondHand)) {
            ...
            return firstHand;
        } else if (isThreeOfAKind(secondHand) && !isThreeOfAKind(firstHand)) {
            return secondHand;
        }
        return noPair;
    }
